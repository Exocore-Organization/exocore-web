...
export function validateEmail(email: string): { ok: true } | { ok: false; message: string } {
  if (!email || typeof email !== "string") return { ok: false, message: "Email is required" };
  const trimmed = email.trim().toLowerCase();
  const re = /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)+$/;
  if (!re.test(trimmed)) return { ok: false, message: "Invalid email format" };
  if (trimmed.length > 254) return { ok: false, message: "Email is too long" };
...
export function validatePassword(pass: string): { ok: true } | { ok: false; message: string } {
  if (!pass || typeof pass !== "string") return { ok: false, message: "Password is required" };
  if (pass.length < 10) return { ok: false, message: "Password must be at least 10 characters" };
  if (pass.length > 128) return { ok: false, message: "Password is too long" };
  if (!/[a-z]/.test(pass)) return { ok: false, message: "Password must include a lowercase letter" };
  if (!/[A-Z]/.test(pass)) return { ok: false, message: "Password must include an uppercase letter" };
  if (!/\d/.test(pass)) return { ok: false, message: "Password must include a number" };
  if (!/[^A-Za-z0-9]/.test(pass)) return { ok: false, message: "Password must include a symbol (e.g. !@#$%)" };
  if (/\s/.test(pass)) return { ok: false, message: "Password cannot contain spaces" };
  return { ok: true };
}
...
export function validateUsername(input: string): { ok: true } | { ok: false; message: string } {
  if (!input || typeof input !== "string") return { ok: false, message: "Username is required" };
  const raw = input.trim();
  if (raw.length < 3) return { ok: false, message: "Username must be at least 3 characters" };
  if (raw.length > 32) return { ok: false, message: "Username is too long (max 32)" };
