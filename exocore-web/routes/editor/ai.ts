...
import {
    chat,
    image as generateImage,
    deleteConversation,
    findWorkingCookies,
    setMode as setConvoMode,
    warmup as warmupConvo,
    reapToOne,
    type CookieMap,
...
export class AiRoute {
    ...
    constructor() {
        ...
        this.activeFile = path.join(this.projectRoot, "config", "llama_active.json");
        this.sessionFile = path.join(this.projectRoot, "config", "llama_session.json");
    ...
    private async extraChat(req: Request, res: Response): Promise<void> {
        ...
        if (!prompt) { res.status(400).json({ ok: false, error: "empty_prompt" }); return; }

        const upstream: Record<string, unknown> = { ...req.body, message: prompt };
    ...
    private loadSession(): { conversationId?: string; mode?: SessionMode } {
        if (!fs.existsSync(this.sessionFile)) return {};
        try { return JSON.parse(fs.readFileSync(this.sessionFile, "utf-8")) || {}; }
        catch { return {}; }
    }
    private saveSession(s: { conversationId?: string; mode?: SessionMode }): void {
        const dir = path.dirname(this.sessionFile);
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(this.sessionFile, JSON.stringify(s, null, 2));
    }
    private clearSession(): void {
        try { if (fs.existsSync(this.sessionFile)) fs.unlinkSync(this.sessionFile); } catch {}
    }

    private loadCookies(): CookieMap | null {
    ...
    private cookiesFromReq(req: Request): CookieMap | null {
        const fromBody = (req as any)?.body?.cookies;
        if (fromBody && typeof fromBody === "object" && !Array.isArray(fromBody)) {
            const cleaned: CookieMap = {};
    ...
    private saveActive(cookies: CookieMap, source: string, perCookie?: any): void {
        const dir = path.dirname(this.activeFile);
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(this.activeFile, JSON.stringify({
    ...
    private status(_req: Request, res: Response): void {
        ...
        res.json({
            ready: !!cookies,
            activeConfig: fs.existsSync(this.activeFile),
            engine: "exocore-llama",
            baseUrl: "https://exocore-llama.hf.space",
        });
    }

    private getActiveCookieMeta(_req: Request, res: Response): void {
        const cookies = this.loadCookies();
        if (!cookies) { res.json({ saved: false }); return; }
        res.json({ saved: true, cookieNames: Object.keys(cookies) });
    }

    private clearCookies(_req: Request, res: Response): void {
        try { if (fs.existsSync(this.activeFile)) fs.unlinkSync(this.activeFile); } catch {}
        res.json({ ok: true });
    }
    ...
    private async saveCookies(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            const { working: probed, perCookie } = await findWorkingCookies(cookies);
            ...
            let working: Record<string, string> = probed;
            ...
            const hasAuthCarrier =
                survived.has("abra_sess") || survived.has("c_user") || survived.has("xs") ||
            ...
            const ping = await chat({ prompt: "hi", cookies: working });
            if (ping.conversationId) {
                deleteConversation({ cookies: working, conversationId: ping.conversationId, accessToken: ping.accessToken })
                    .catch(() => { /* best-effort */ });
            }
            res.json({
                ok: true,
    ...
    private async chat(req: Request, res: Response): Promise<void> {
        ...
        if (this.inFlight) {
            try { await this.inFlight; } catch { /* ignore previous failure */ }
        }
        ...
        try {
            ...
            const survivor = await reapToOne(cookies).catch(() => undefined);
    ...
    private getSession(_req: Request, res: Response): void {
    ...
    private async resetSession(req: Request, res: Response): Promise<void> {
        ...
        if (s.conversationId && cookies) {
            try { await deleteConversation({ cookies, conversationId: s.conversationId }); }
            catch { /* best-effort */ }
        }
        this.clearSession();
        res.json({ ok: true });
    ...
    private async createSession(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            const before = await reapToOne(cookies).catch(() => undefined);
            if (before) {
                try { await deleteConversation({ cookies, conversationId: before }); }
                catch { /* best-effort */ }
            }
            const result = await chat({
                prompt: seed,
            ...
            const id = (await reapToOne(cookies).catch(() => undefined))
    ...
    private async changeMode(req: Request, res: Response): Promise<void> {
        const mode = req.body?.mode as SessionMode | undefined;
        const VALID: SessionMode[] = ["CHAT", "IMAGINE", "think_fast", "think_hard"];
        ...
        const cookies = this.cookiesFromReq(req);
        if (!cookies) { res.status(503).json({ ok: false, error: "no_cookies" }); return; }
        const s = this.loadSession();
        const id = (typeof req.body?.id === "string" && req.body.id) || s.conversationId;
        if (!id) { res.status(409).json({ ok: false, error: "no_session", detail: "No active conversation. Send a chat first." }); return; }

        const r = await setConvoMode({ cookies, conversationId: id, mode });
        if (r.success) this.saveSession({ ...s, conversationId: id, mode });
    ...
    private async warmupSession(req: Request, res: Response): Promise<void> {
        const cookies = this.cookiesFromReq(req);
        if (!cookies) { res.status(503).json({ ok: false, error: "no_cookies" }); return; }
        const s = this.loadSession();
        const id = (typeof req.body?.id === "string" && req.body.id) || s.conversationId;
        if (!id) { res.status(409).json({ ok: false, error: "no_session", detail: "No active conversation. Send a chat first." }); return; }
        const r = await warmupConvo({ cookies, conversationId: id });
        res.json({ ok: r.success, conversationId: id, raw: r.raw });
    }

    private async image(req: Request, res: Response): Promise<void> {
        const prompt = typeof req.body?.prompt === "string" ? req.body.prompt.trim() : "";
        ...
        try {
            const result = await generateImage({ prompt, cookies, orientation });
    ...
    private async agent(req: Request, res: Response): Promise<void> {
        const prompt = typeof req.body?.prompt === "string" ? req.body.prompt.trim() : "";
        ...
        const agentMode: any = rawAgentMode === "thinking" ? "think_hard"
        ...
        if (projectId) {
            ...
            const root = path.join(this.projectsDir, safe);
                        ...
                        if (ent.isDirectory()) {
                            ...
                            if (TEXT_EXT.has(ext) && collected.length < 20) {
                                try {
                                    ...
                                    if (stat.size < 40_000) collected.push(rel);
                                } catch {}
                            }
                        }
                    }
...
Your previous attempt was rejected because it was prose, not JSON, or had zero actions. Reply with the JSON plan only this time.`;
    ...
    private async cleanupConversations(req: Request, res: Response): Promise<void> {
        ...
        const results = await Promise.all(
            (ids as unknown[])
                ...
                .map(async (id) => {
                    try {
                        const r = await deleteConversation({ cookies, conversationId: id });
    ...
    private async deleteAll(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            const r = await axios.post(
                (process.env.EXOCORE_LLAMA_BASE_URL || "https://exocore-llama.hf.space") + "/delete/all",
    ...
    private async deleteConvo(req: Request, res: Response): Promise<void> {
        const conversationId = typeof req.body?.conversationId === "string" ? req.body.conversationId : "";
        if (!conversationId) { res.status(400).json({ ok: false, error: "missing_conversationId" }); return; }
        const cookies = this.cookiesFromReq(req);
        if (!cookies) { res.status(503).json({ ok: false, error: "no_cookies" }); return; }
        try {
            const r = await deleteConversation({ cookies, conversationId });
...
function extractJsonPlan(reply: string): ParsedPlan & { parsed: boolean } {
    const fallback = { message: reply.trim() || "(empty response)", actions: [] as ParsedPlan["actions"], parsed: false };
