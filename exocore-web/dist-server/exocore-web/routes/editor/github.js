...
class GithubRoute {
    ...
    async ensureGitIgnore(projectDir) {
        const ignorePath = path_1.default.join(projectDir, ".gitignore");
        try {
            const content = await promises_1.default.readFile(ignorePath, "utf8");
            if (!content.includes("node_modules")) {
                await promises_1.default.appendFile(ignorePath, "\nnode_modules\n.env\n.DS_Store\n");
            }
        }
        catch {
            await promises_1.default.writeFile(ignorePath, "node_modules\n.env\n.DS_Store\n");
        }
    }
    requestDeviceCode = async (_req, res) => {
        try {
    ...
    getUserRepos = async (req, res) => {
        ...
        try {
            const response = await axios_1.default.get("https://api.github.com/user/repos?per_page=100&sort=updated", {
    ...
    gitStatus = async (req, res) => {
        ...
        try {
            await promises_1.default.access(path_1.default.join(projectDir, ".git"));
            let remote = "";
            try {
            ...
            let trackedFiles = [];
            try {
                const { stdout: trackedOut } = await execAsync(`git ls-tree -r HEAD --name-only`, { cwd: projectDir });
                ...
                try {
                    const { stdout: filesOut } = await execAsync(`git ls-files`, { cwd: projectDir });
    ...
    getProjectChanges = async (req, res) => {
        const { projectId } = req.query;
        if (!projectId)
            return res.status(400).json({ error: "missing_project" });
        const projectDir = this.getFullPath(projectId);
        try {
            ...
            try {
                const entries = await promises_1.default.readdir(projectDir, {
                    withFileTypes: true,
                });
                const files = entries
                    .filter((e) => (e.name === ".gitignore" ||
                    !e.name.startsWith(".")) &&
                    e.name !== "node_modules")
    ...
    cloneRepo = async (req, res) => {
        ...
        try {
            if (extract) {
                const tmpFolder = path_1.default.join(projectDir, "_temp_clone");
                await execAsync(`git clone "${repoUrl}" "${tmpFolder}"`);
                const files = await promises_1.default.readdir(tmpFolder);
                for (const file of files)
                    await promises_1.default.rename(path_1.default.join(tmpFolder, file), path_1.default.join(projectDir, file));
                try {
                    await promises_1.default.rename(path_1.default.join(tmpFolder, ".git"), path_1.default.join(projectDir, ".git"));
                }
                catch { }
                await promises_1.default.rm(tmpFolder, { recursive: true, force: true });
            }
            else {
    ...
    createRepo = async (req, res) => {
        const { projectId, token, username, email, repoName, isPrivate, files, } = req.body;
        const projectDir = this.getFullPath(projectId);
        try {
            await axios_1.default.post("https://api.github.com/user/repos", { name: repoName, private: isPrivate }, { headers: { Authorization: `token ${token}` } });
            ...
            await execAsync(`git remote add origin "${authUrl}"`, {
                cwd: projectDir,
            }).catch(() => execAsync(`git remote set-url origin "${authUrl}"`, {
                cwd: projectDir,
            }));
    ...
    connectRepo = async (req, res) => {
        ...
        try {
            ...
            await execAsync(`git pull origin main --rebase`, {
                cwd: projectDir,
            }).catch(() => execAsync(`git pull origin main --allow-unrelated-histories`, {
                cwd: projectDir,
            }).catch(() => { }));
            res.json({ success: true });
        }
    ...
    pushRepo = async (req, res) => {
        ...
        try {
            ...
            const safeMsg = (commitMsg || "Update via Exocore IDE").replace(/"/g, '\\"');
...
exports.GithubRoute = GithubRoute;
