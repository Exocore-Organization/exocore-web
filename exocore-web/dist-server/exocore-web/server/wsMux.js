...
exports.createMuxWss = createMuxWss;
...
class MuxedSocket extends events_1.EventEmitter {
    _send;
    _close;
    static CONNECTING = 0;
    static OPEN = 1;
...
function encodeFrame(type, ch, payload) {
    ...
    if (payload && payload.length)
        Buffer.from(payload.buffer, payload.byteOffset, payload.byteLength).copy(out, 2 + name.length);
    return out;
}
function encodeClose(ch, code, reason) {
...
function createMuxWss(hubs) {
    const carrierWss = new ws_1.WebSocketServer({ noServer: true, maxPayload: 32 * 1024 * 1024 });
    carrierWss.on("connection", (carrier, req) => {
        const channels = new Map();
        const sendRaw = (frame) => {
            if (carrier.readyState !== ws_1.WebSocket.OPEN)
                return;
            try {
                carrier.send(frame);
            }
        ...
        carrier.on("message", (raw) => {
            ...
            if (!hub)
                return;
            if (m.type === FT_OPEN) {
                if (channels.has(m.ch))
                    return;
                const synthReq = Object.create(req);
                synthReq.url = m.payload.length ? m.payload.toString("utf-8") : `/${m.ch}`;
                const fake = new MuxedSocket((bytes) => sendRaw(encodeFrame(FT_DATA, m.ch, bytes)), (code, reason) => sendRaw(encodeClose(m.ch, code, reason)));
                channels.set(m.ch, fake);
                hub.emit("connection", fake, synthReq);
                return;
            }
