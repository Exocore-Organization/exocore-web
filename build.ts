...
const srcExists = await Deno.stat(`${SOURCE_DIR}/packages/index.ts`).then(() => true).catch(() => false);
if (!srcExists) {
  console.error(`[build] Source not found at ${SOURCE_DIR}/`);
  console.error("Expected: backup/exocore-ide/packages/index.ts");
  Deno.exit(1);
}
